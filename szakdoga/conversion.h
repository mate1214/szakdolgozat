...
namespace approx{
	...
	//egyetlen test adatainak kinyerese
	template <class T> BodyList drawinfo(const Body<T>& body){
		BodyList res;
		const std::vector<Vec3>& vs = *body.faces(0).vertex_container();
		const std::vector<Vec3>& ns = *body.faces(0).normal_container();
		...
		for (const Face<T>& f : body){
			for (int i = 2; i < f.size(); ++i){
				...
				res.normals.push_back(f.normal_index());
				res.normals.push_back(f.normal_index());
				res.normals.push_back(f.normal_index());
			}
		}
	...
	//testek kozos adatai egy csomagban
	template <class BodyIterator,class T> BodyList drawinfo(BodyIterator first, BodyIterator last){
		BodyList res;
		const Body<T>& b = *first;
		const std::vector<Vec3>& vs = *b.faces(0).vertex_container();
		const std::vector<Vec3>& ns = *b.faces(0).normal_container();
		res.points.reserve(vs.size());
		for (const Vector3<T> p : vs){
			res.points.push_back(Vec3(p.x, p.y, p.z));
		}
